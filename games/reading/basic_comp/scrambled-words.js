...
let currentStage = null;
 ...
 const gameData = [
     ...
     },
     {
         ...
         words: [
             ...
             { text: "turtle", type: "other", order: -1 }
         ]
     }
 ];
 ...
 function renderLevel(stage) {
     ...
     stage.innerHTML = `
         <style>
             .sentence-container { display: flex; flex-direction: column; align-items: center; gap: 20px; padding: 20px; font-family: 'Segoe UI', 'Comic Sans MS', 'OpenDyslexic', system-ui, sans-serif; max-width: 800px; margin: 0 auto; color: #2D3748; }
             .game-header { display: flex; justify-content: space-between; align-items: center; width: 100%; padding: 12px 20px; background: #EDF2F7; border-radius: 12px; }
             .level-badge { background: ${badgeColor}; color: white; padding: 6px 14px; border-radius: 20px; font-size: 0.9rem; font-weight: bold; letter-spacing: 0.05em; }
             ...
             .instruction-text { font-size: 1.15rem; color: #4A5568; text-align: center; margin: 5px 0 15px 0; font-weight: 600; background: #FFF; padding: 12px 20px; border-radius: 10px; border-left: 5px solid ${badgeColor}; width: 100%; box-sizing: border-box; }
             .magnetic-rail { display: flex; flex-wrap: wrap; gap: 12px; padding: 25px; background: #F7FAFC; border: 3px dashed #CBD5E0; border-radius: 16px; min-height: 100px; width: 100%; justify-content: center; align-items: center; box-sizing: border-box; }
             .word-slot { width: 110px; height: 65px; border-bottom: 4px solid #E2E8F0; display: flex; align-items: center; justify-content: center; transition: all 0.3s ease; position: relative; }
             .word-slot::before { content: attr(data-slot-num); position: absolute; top: -25px; font-size: 0.85rem; color: #A0AEC0; font-weight: bold; }
             .word-slot.drag-over { background: #EBF8FF; border-bottom-color: #4299E1; }
             .magnet-pool { display: flex; flex-wrap: wrap; gap: 15px; margin-top: 30px; justify-content: center; padding: 25px; background: #EDF2F7; border-radius: 16px; width: 100%; box-sizing: border-box; min-height: 100px; }
             .magnet { padding: 12px 20px; background: white; border: 2px solid #E2E8F0; border-radius: 10px; font-size: 1.15rem; font-weight: 700; cursor: grab; box-shadow: 0 4px 6px rgba(0,0,0,0.05); touch-action: none; z-index: 10; user-select: none; letter-spacing: 0.03em; transition: transform 0.2s, box-shadow 0.2s; }
             ...
             .magnet.correct { cursor: default; box-shadow: 0 0 0 3px rgba(72, 187, 120, 0.3); border-color: #48BB78 !important; background: #F0FFF4; transform: none !important; }
             .shaking { animation: shakeMagnet 0.4s ease-in-out; }
             @keyframes shakeMagnet { 0%, 100% { transform: translateX(0); } 20% { transform: translateX(-6px); } 40% { transform: translateX(6px); } 60% { transform: translateX(-4px); } 80% { transform: translateX(4px); } }
         </style>
